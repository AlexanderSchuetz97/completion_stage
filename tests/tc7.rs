...
use completion_stage::{Completion, CompletionStage, TakenCompletion};
...
fn test() {
    ...
    let v1 = n.then_apply(|comp| {
        assert_eq!(comp, TakenCompletion::DeadLock); //Because joined is borrowed, we cannot take it out.
        return Completion::Value(456)
    }).take().unwrap();
    assert_eq!(v1, 456);
    drop(joined);
    let v2 = n.then_apply(|comp| {
        assert_eq!(comp, TakenCompletion::Value(()));
...
fn test2() {
    CompletionStage::new_async_with_executor((), || {
        ...
        n.then_apply(move |comp| {
            ...
            assert!(matches!(comp, TakenCompletion::Value(_)));
